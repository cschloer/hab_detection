...
# else:
# This if/else is being used post development to make it easier to change image generation

# json.dump([scenes[key] for key in used_tiles], f)
with open(f"/home/conrad/Dropbox/Masters/masters_thesis/data/data.json", "r") as f:
    scenes_list = json.load(f)
    designation_scenes = {}
    for scene in scenes_list:
        designation_scenes[scene["id"]] = scene
least_clouds = {
    "8_3": {"download_path": "/tmp/hab/8_3_241.tif", "percent_clouds": 0.630338},
    "6_2": {"download_path": "/tmp/hab/6_2_221.tif", "percent_clouds": 0.01716475},
    "7_2": {"download_path": "/tmp/hab/7_2_237.tif", "percent_clouds": 0.02473375},
    "7_5": {"download_path": "/tmp/hab/7_5_196.tif", "percent_clouds": 0.4115955},
    "6_5": {"download_path": "/tmp/hab/6_5_198.tif", "percent_clouds": 0.693113},
}
...
for region_id in region_ids:
    ...
    for x in range(0, CYAN_SIZE, SCENE_SIZE):
        for y in range(0, CYAN_SIZE, SCENE_SIZE):
            scene_id = get_scene_id(region_id, x, y)
            scene = designation_scenes.get(scene_id, None)
            if scene and "ignore" not in scene:
                print(f"got a scene {scene_id}")
    ...
    ax.axis("off")
    plt.savefig(f"{SAVE_FOLDER}/images/{region_id}.pdf", dpi=900, bbox_inches="tight")
    plt.savefig(f"{SAVE_FOLDER}/images/{region_id}.png")
    plt.savefig(f"{SAVE_FOLDER}/images/{region_id}.svg", format="svg")
