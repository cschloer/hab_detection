...
class ImageData(Dataset):
    ...
    def transform_label(self, label):
        ...
        for i, ceil in enumerate(self.class_designation):
            print(f"{i} TRANSFORMING 254", label.count_nonzero(label == 254))
            print(f"{i} TRANSFORMING 255", label.count_nonzero(label == 255))
            label = np.where((label >= floor) & (label < ceil), i, label)
            floor = ceil
        print(f"DONE TRANSFORMING 254", label.count_nonzero(label == 254))
        print(f"DONE TRANSFORMING 255", label.count_nonzero(label == 255))
